...
import { X, ShoppingBag, Download, Check, Box, LogOut } from "lucide-react";
...
export const Product3DModal = ({
  ...
  const files = Array.isArray(product.modelFiles)
    ? product.modelFiles
        .filter(
          (file): file is { format: string; url?: string } =>
            !!file &&
            typeof file.format === "string" &&
            file.format.trim().length > 0,
        )
        .map((file) => ({
          ...file,
          format: file.format.trim().toUpperCase(),
        }))
    : [];
  ...
  const handleDownload = (format: string) => {
    if (!isAuthenticated || !token) {
      toast.info("Login required for 3D download", {
    ...
    fetch(
      `${API_URL}/api/models/${encodeURIComponent(product.id)}/download?format=${encodeURIComponent(format)}`,
      {
        method: "GET",
      ...
      .then(async (res) => {
        ...
        if (res.status === 403) {
          toast.error("Download not allowed", {
            description:
              "This model requires both confirmed purchase and admin access grant for your account.",
          });
          return null;
        }
        if (!res.ok) {
          toast.error("Download unavailable right now.");
          return null;
        }
      ...
      .catch((error) => {
        console.error("Failed to request model download:", error);
                ...
                <div className="flex flex-col md:flex-row max-h-[85vh]">
                  ...
                  <div className="flex-1 flex flex-col overflow-y-auto">
                    <div className="p-4 sm:p-5 md:p-6 lg:p-8 flex flex-col h-full">
                      <div className="flex justify-between items-start mb-3 sm:mb-4 md:mb-5">
                        <div>
                          ...
                          <p className="text-[10px] sm:text-xs text-neutral-400 mt-0.5">
                            {hasFiles
                              ? `${files.length} format${files.length > 1 ? "s" : ""} available`
                              : "No 3D files configured yet"}
                          </p>
                        </div>
                      ...
                      <p className="text-xs sm:text-sm font-medium text-neutral-800 mb-0.5">
                        {product.name}
                      </p>
                      <p className="text-[10px] sm:text-xs text-neutral-500 mb-3 sm:mb-4 md:mb-5">
                        {product.category} &middot; Br{" "}
                        {safePrice.toLocaleString()}
                      </p>

                      {!isAuthenticated ? (
                        <div className="mb-3 sm:mb-4 md:mb-5 rounded-xl border border-neutral-200 bg-neutral-50 p-3 sm:p-4">
                          <p className="text-[11px] sm:text-xs font-bold uppercase tracking-[0.2em] text-neutral-500 mb-2">
                            3D Client Access Required
                          </p>
                          <p className="text-[10px] sm:text-xs text-neutral-500 mb-3">
                            Please use the dedicated 3D access page to login or
                            register. Website browsing remains public.
                          </p>
                          <button
                            type="button"
                            onClick={() => {
                              window.location.href = "/3d-access";
                            }}
                            className="w-full rounded-md bg-neutral-900 px-3 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-white hover:bg-neutral-800"
                          >
                            Go to 3D access page
                          </button>
                        </div>
                      ) : hasFiles ? (
                        <>
                          <div className="mb-2.5 flex items-center justify-between gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-2">
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1 text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-emerald-800 hover:text-emerald-900"
                            >
                              ...
                              return (
                                ...
                                >
                                  <div className="flex-1 min-w-0">
                                    <span className="text-[10px] sm:text-xs font-bold text-neutral-800 block">
                                      .{extension}
                                    </span>
                                    <span className="text-[9px] sm:text-[10px] text-neutral-500 block mt-0.5 leading-tight">
                                      {desc}
                                    </span>
                                  </div>
                                  <Download
                                    size={12}
                                    className={`shrink-0 mt-0.5 sm:w-3.5 sm:h-3.5 ${isSelected ? "text-amber-500" : "text-neutral-300"}`}
                                  />
                                  {isSelected && (
                                    <div className="absolute top-1 right-1 sm:top-1.5 sm:right-1.5">
                                      <Check
                                        size={10}
                                        className="sm:w-3 sm:h-3 text-amber-500"
                                      />
                                    </div>
                                  )}
                                </button>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center py-6">
                          ...
                          <p className="text-sm text-neutral-500">
                            No 3D model files available for this product yet.
                          </p>
                          <p className="text-xs text-neutral-400 mt-1">
                            3D files can be added from the admin dashboard.
                          </p>
                        </div>
                      )}
                      ...
                      >
                        ...
                        Add to Cart — Br {safePrice.toLocaleString()}
                      </button>
